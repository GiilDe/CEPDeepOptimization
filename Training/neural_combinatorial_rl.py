...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context):
        ...
        for _ in range(self.max_length):
            ...
            decoder_input = embedded_inputs[idxs.data, [i for i in range(batch_size)], :]
...
class NeuralCombOptNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.actor_net = PointerNetwork(
            embedding_dim if embedding_dim != 0 else input_dim,
            hidden_dim,
            max_decoding_len,
            n_glimpses,
            tanh_exploration,
    ...
    def forward(self, inputs):
        ...
        # Select the actions (inputs pointed to by the pointer net) and the corresponding logits
        # should be size [batch_size]
        actions = []
        # inputs is [batch_size, input_dim, source_length]
        inputs_ = inputs.transpose(1, 2)
        # inputs_ is [batch_size, source_length, input_dim]
        for action_id in actions_idxs:
        ...
        if self.is_train:
            ...
            # return the list of len source_length of [batch_size, source_length]
            probs = probs_
        ...
        return probs, actions, actions_idxs
